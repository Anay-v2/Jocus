...
import GameJoinIndicator from '@/components/GameJoinIndicator.vue';
    ...
    const sgame = ref(games.find(g => g.id === route.params.id))
    ...
    <main>
		<div class="grid place-items-center w-screen h-[90vh]" v-if="game.status === 'n'">
			<div
				...
				<button class="btn btn-primary" :disabled="!(game.joined?.length || 0 >= (sgame?.players[0] || 1) && game.joined?.length || 0 <= (sgame?.players[1] || Infinity))" v-if="auth.currentUser?.uid === game.creator"><Play /> Start Game</button>
			</div>
		</div>
    </main>
</template>